...
public class CompraDAO {
    ...
    public List<List<String>> listaPrestadores(String categoria) {
        ...
        String query = "SELECT DISTINCT u.nome, pt.login, u.endereco FROM Prestador pt INNER JOIN Produto p ON pt.login = p.loginPrestador " +
        ...
        try (PreparedStatement stmt = ConnectionFactory.getDBConnection().prepareStatement(query);
            ...
            while(rs.next()) {
                ...
                prestador.add(rs.getString("endereco"));
     ...
     * @param idCompra
     */
    public void listaDeProdutos(int idProduto, int idCompra, int quantidade) {

        String query = "INSERT INTO Lista_Produtos (idProduto,idCompra,quantidade) VALUES (?,?,?)";
    ...
    /**
     * Nome e preco do produtos escolhido
     * @param id
     * @return nome e preco
     */
    public String[] nomePrecoProduto(int id) {
        String query = "SELECT nome, preco FROM Produto WHERE id = " + id;
    ...
    /**
     * Descrição Geral do Prestador
     * @param loginPrestador
     * @return Descrição Geral
     */
    public String desGeralPrestador(String loginPrestador) {
        String query = "SELECT descGeral FROM Prestador WHERE login = \'" + loginPrestador + "\'";
        String desc = "";
        try (PreparedStatement stmt = ConnectionFactory.getDBConnection().prepareStatement(query);
             ResultSet rs = stmt.executeQuery()) {
            if (rs.next()) {
                desc = rs.getString("descGeral");
            } else {
                desc = "Sem descrição";
            }
        } catch (SQLException e) {
